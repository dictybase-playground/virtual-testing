...
<Image src={virtualization} height={400} />
...
- Create one `useEffect` that fetches more data if `intersecting` is true
- Add the target ref to the last row of the `rowData` provided by `useVirtualList`
- Add a parent ref to the outer container

That's it!

[Code example](https://github.com/dictybase-playground/catalog-search/blob/feature/virtualization/src/CatalogList.tsx)
